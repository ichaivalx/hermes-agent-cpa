...
- 自定义补丁版本：`23`
- 镜像：`ghcr.io/ichaivalx/hermes-agent-cpa:v2026.8.13-cpa.23`
...
13. 锚点被腾讯拒绝时先判定为不可重试，不再重复三次同样注定失败的请求，然后去掉 `msg_id` 重投一次；腾讯已放开不带引用的主动消息，这一次成功就把原本会静默丢失的回复发出去。文档给出的错误码是 `304027`，实际群聊返回的是 `[400] code=40034031 ... msgid已经过期,不能回复`，被内容审核拦下的入站图片也走同一个错误，因此两种措辞都被识别；`access_token已过期` 之类无关的过期错误不会被误判成锚点问题。回退只针对这一类失败、只在原本确实带了锚点时触发、并且只重投一次（单次尝试、不做退避重试），被拒的主动消息因此不会成倍消耗每日推送额度。重投同样失败时返回不可重试并在错误里保留两次失败的原文，`40034102`（主动消息失败／无权限）也判定为不可重试。只覆盖文本发送路径，媒体与按钮发送仍是原行为。
...
```yaml
image: ghcr.io/ichaivalx/hermes-agent-cpa:v2026.8.13-cpa.23
```
