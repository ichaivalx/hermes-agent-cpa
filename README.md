...
- 自定义补丁版本：`31`
- 镜像：`ghcr.io/ichaivalx/hermes-agent-cpa:v2026.8.13-cpa.31`
...
5. `direct` 模式下明确 @ 的消息走同一条可见发送边界：它的普通 final、流式片段和中途文本同样不外发，也必须调用发送工具才能说话。区别只在于 @ 回合保留全部交互面（审批、Clarify、输入中提示、工具进度、排队与卡死通知），普通未 @ 消息则连这些也一并静默。这样一个共享群会话内不再存在两套投递契约，模型也不会把「本以为是私有想法」的一段话在 @ 回合直接播出去。这条边界只针对模型写出来的文本：网关在 Agent 启动前就返回的文本一律照常发出，包括 `@机器人 /new` 的「新会话已开始」回执、`/status` 输出、斜杠命令权限拒绝、忙碌与排队拒绝提示和审批提示——它们不可能含有模型输出。判定方式是网关在真正调用 Agent 前给该事件打一个标记，末端投递门只抑制带标记的文本，因此不需要逐个枚举命令。网关自身产生的失败提示（`⚠️ …`）同理：@ 回合仍会发出，避免每次 API 报错都变成对提问者的沉默；未 @ 回合继续静默失败，普通群消息本来也不会触发斜杠命令。
6. 两个发送工具都没有群 ID、用户 ID 或其他目标参数，目标由当前入站事件固定；脱离该普通群消息的 Agent turn 就会失败关闭，因此不能跨群或从私聊误发。媒体工具只接受当前 Profile 的生成缓存、生成服务返回的 HTTP(S) 媒体 URL，或当前群 Workspace 内的文件（包括 `incoming/`），不接受任意服务器路径。
7. 对腾讯可能重复投递的普通事件与 @ 事件做升级式去重；先写入的被动副本会在显式 @ 到达前原子删除。腾讯全群事件中的结构化机器人提及会依据事件自带的 `bot` / `is_you` 标记精确移除，因此明确 `@机器人 /new`、`@机器人/status` 等命令仍由网关执行；普通未 @ 消息里的斜杠文本继续交给 Agent，不会变成管理命令。
8. Agent 的工具调用、发送内容和私有最终回复均保存在正常会话历史中；腾讯回流的机器人事件只作为重复副本丢弃，不会让机器人对自己的消息再次触发 Agent。
9. 全群模式强制要求精确群 ID 白名单和共享群会话；`*` 通配符不会开启普通消息采集。
10. 群聊可以设置独立 `group_toolsets`，包括显式空列表；普通群文本不能执行 Hermes 斜杠命令。
11. `direct` 普通消息和明确 @ 消息的模型提示可按 Profile 完整替换或关闭；结构化上下文处理与可见发送边界不依赖这段可编辑文本。
12. `qq_group_send` / `qq_group_send_media` 交给模型看的工具描述也可按 Profile 替换或清空，用于消除内置描述里的措辞与自定义人格互相打架的问题。只有描述文本可配置，参数契约、固定目标和全部投递边界仍在代码中。
13. 可见发送在真正发出前重新读取该群最新的入站 `msg_id` 作为引用锚点。腾讯要求群消息引用一条较新的入站消息，思考较久的 `direct` turn 若仍引用自己那条触发消息就会发送失败；改为发送时取最新锚点后，目标群不变，只有引用对象换成群里最近的一条消息。
14. 锚点被腾讯拒绝时先判定为不可重试，不再重复三次同样注定失败的请求，然后去掉 `msg_id` 重投一次。这条回退在生产环境已经实测生效：2026-08-18 连续三次把原本会静默丢失的群回复发了出去，且从未收到 `40034102`，说明该机器人的主动消息权限是放开的。文档给出的错误码是 `304027`，实际群聊从不返回它，而是用两个不同的错误码配两种不同措辞表达同一件事——`[400] code=40034031 ... msgid已经过期,不能回复` 与 `[400] code=40034005 ... 回复消息msg_id已过期`。注意一个写 `msgid`、另一个写 `msg_id`：判定只做字面子串匹配，这一个下划线就足以让标记落空（`msgid已过期` 不是 `msg_id已过期` 的子串），因此匹配前会从错误文本中去掉下划线，并且表内所有标记都不含下划线，让 `msgid` / `msg_id` / `msg id` 三种写法归一，上游改写字段拼法也不会再静默失效；有一条测试钉死这个不变量。`access_token已过期` 之类无关的过期错误不会被误判成锚点问题。`40034006 消息内容违规` 刻意不在表内：那是腾讯拒绝消息内容本身，去掉锚点重投同一段内容只会再失败一次并白耗推送额度。回退只针对锚点这一类失败、只在原本确实带了锚点时触发、并且只重投一次（单次尝试、不做退避重试）。重投同样失败时返回不可重试并在错误里保留两次失败的原文，`40034102`（主动消息失败／无权限）也判定为不可重试。按钮发送仍是原行为；媒体发送路径见第 15 条。
15. 媒体发送（图片／视频／语音／文件）遇到同一类锚点过期时，不重新上传，只把已经拿到的 `file_info` 去掉 `msg_id` 再 POST 一次。这条路径比文本更容易撞上：出图本身常要几十秒到两分钟，而锚点只活几分钟；生产 2026-08-19 16:30 就是一张 665 KB 的图上传成功后被 `40034005` 拒绝，然后整个分块上传又白跑了一遍、再被拒绝一次，最后靠群里恰好有人说话刷新了锚点才发出去，白等三分半。因为上传本身是成功的、只有最后那条可见消息被拒，所以重投复用同一个上传令牌，既不再传一遍字节也不多占腾讯的每日上传额度；每次 POST 取新的 `msg_seq`，避免重投和刚失败的那次完全同形。约束与文本路径一致：只针对锚点这一类失败（`40034006 消息内容违规` 之类照旧直接失败）、只在原本确实带了锚点时触发、只重投一次；两次都失败时返回不可重试并保留两次失败的原文。重投前会再查一次本回合是否已被 `/stop` 或新回合取代，取消了就不发。媒体路径整体仍然不做自动重试——媒体重试意味着重跑整个分块上传，为一个重复也修不好的错误再推一遍几 MB 没有意义。
16. 群聊 `direct` 且 `busy_input_mode: steer` 时，中途到达的图片不再只进下一轮队列：先把「谁、何时、哪张图、本地路径」写进当前 turn 的 Steer 文本（时间戳无条件渲染，不依赖 `gateway.message_timestamps`），像素仍在下一轮以原生附件到达，避免后到的纯文字越过先到的图。媒体配文里的 `<@openid>` 会拆成单独一条 markdown 发出去，因为 QQ 的 `msg_type=7` 配文不解析 @。可选 `platforms.qqbot.extra.media_placeholder_note` 在占位符后追加引导，默认空。
17. 共享群会话里「谁在什么时候发了哪张图」在整条链路上对齐。根因在事件层：模型忙的时候，同一个群里多个人的消息会被合并进一个待处理事件，合并只把文字拼在一起、把媒体 URL 追加到同一个列表，于是发言人和时间在拼接中被抹平——后台面板里看到的就是顶上一串 `[昵称|openid]` 前缀、底下一摞与任何人都无关的 `[Image attached at: …]`。现在合并前先给每一条被并进来的消息记一份来源（发言人、时间戳、原文、以及它贡献的媒体区间 `media_start`/`media_end`），交给模型时再按这份记录展开：每条消息各自带回自己的 `[时间] [昵称|openid]` 前缀，顺序与到达顺序一致。图片则按媒体区间反查归属，为每张图单独生成一行 `[时间] [某人 sent this image: 路径]`，并且和像素交错排列——一行归属紧跟它描述的那张图，而不是把归属堆在文字块末尾，因此模型不可能把张三的图当成李四发的。同时补齐所有出站路径上缺失的时间戳：普通 Steer 文本、优先图片 Steer、两处 `redirect()`、三处 `interrupt()`、两处兜底插话以及积压队列排空，全部在调用点渲染时间戳，因为这几条路只传一个字符串、原始事件不会跟到下游，没有别的地方能补。渲染是幂等的（重复渲染只保留离正文最近的那个 epoch），所以多条路径叠加也不会出现两个前缀。另外三处顺带修掉：观察上下文超长时改为截尾并标注 `…`，此前截头会把开头的时间戳和发言人前缀一起丢掉，留给模型一段无主无时间的残句；优先图片快速路径改走统一的排队入口，此前的裸合并默认 `merge_text=False`，会直接覆盖待处理槽位丢掉上一条消息；插话深度到上限时把事件推回 FIFO 而不是丢弃。落库侧只存干净文本：时间戳只写在数据库的 `timestamp` 列、不写进 `content`，避免重放时前缀逐轮累积；`[screenshot]` 占位符只在正文确实没有交代这些图时才补，图片已带归属行时不再追加。
18. 模型忙的时候插进来的纯文字不再静默丢失。这是上游的判定，不是本仓库改出来的，但 0018 让它从偶发变成天天发生：`steer` 模式下中途到达的插话先存在 `_pending_steer`；如果这一轮在最后一次工具调用之后才结束（`finish_reason=stop`），就没有工具结果可以承载它，于是由 `turn_finalizer` 交回给网关作为下一轮用户输入。问题在于网关那一步要求队列完全为空（`if result and not pending and not pending_event`）——而只要中途有图片，第 16 条就会把它的事件留在待处理槽位里等着下一轮原生投递，这个条件必然不成立，插话于是被直接丢掉：没有日志、没有落库、没有任何报错，因为所有相关分支都只打 DEBUG，生产环境不开。表现就是模型回答了图片、完全没提问题，而群友以为自己说过的话机器人看见了。生产 2026-08-21 09:46 复现：三张图分别在 09:40:57 / 09:41:07 / 09:41:23 排队，09:46 的一句纯文字提问就此消失，`gateway.log` 在 09:41:23 到 09:47:16 之间没有任何入站记录。现在队列非空时不再丢弃，而是把插话并进即将排空的那条消息：文字前置（插话确实发生在排空之前，顺序与到达顺序一致），图片仍归属原事件。合并事件要额外记一条来源片段——第 17 条的展开逻辑按片段重建模型看到的文本、完全忽略 `event.text`，只改文本会被它二次丢弃，而这恰好就是最容易触发本问题的那种图片突发。插话文本此前已由 `_prepare_busy_steer_text` 打过时间戳和发言人，因此这条片段不再重复渲染。万一事件对象拒绝写入，插话退回自己单独一轮，同时把那条待处理消息按 FIFO 推回队列——这个分支存在的目的就是不丢消息，不能用一种丢失换另一种。
19. 补上两条 INFO 日志，让下一次这类问题不必靠数据库反推。忙碌路径上的消息不会经过冷路径那条 `inbound message` 日志，而忙碌分支全部只打 DEBUG，于是「模型在忙时收到了什么」在生产日志里完全空白——上面那次排查只能靠 `state.db` 的行来重建时间线。现在 `_handle_active_session_busy_message` 在解析出 `busy_input_mode` 后记录平台、群、会话、模式、消息类型、媒体数量和字符数；QQ 适配器的去重丢弃也从 DEBUG 提到 INFO 并注明命中的是 `addressed` 还是 `observed` 模式。第 18 条修好之后这类丢弃本身就会变少，所以这两条不会明显增加日志量。
  ...
  qqbot:
    ...
    extra:
      ...
      group_prompts:
        ...
        # direct 模式下 @ 消息也是工具专属投递，所以自定义 addressed 必须
        # 自己说明「要说话得调用发送工具」；observe 模式可以删掉最后两行。
        addressed: |
...
- `direct`：每条普通消息直接运行完整 Agent。普通 final 永远不会发到群里；Agent 只有主动调用当前群专用的 `qq_group_send` / `qq_group_send_media` 才会可见发言或发送附件，不调用即保持安静。明确 @ 的消息也走这条同样的边界，只是保留审批、Clarify、输入中提示和工具进度这些交互面。
...
`addressed` 的内置默认值按模式分两份：`observe` 下 @ 消息走 Hermes 原生投递，默认文案不提发送工具；`direct` 下 @ 消息也是工具专属投递，默认文案会附上同一段发送契约。**如果你在 `direct` 模式自己覆盖 `addressed`，必须在自己的文案里说明「要说话得调用 `qq_group_send`」**，否则模型会写一段普通 final，而群里什么都看不到。这种情况会在网关日志里留下一条 WARNING（`Addressed group turn made no visible send but kept a N-char private final`），可以据此定位。

这两个提示只控制模型如何理解群聊、何时选择发言，不承担权限职责。即使自定义提示写错，`direct` 的普通 final、流式文本、中途文本、工具进度和错误细节仍不会直接发到 QQ——明确 @ 的回合也一样；只有请求作用域内、目标固定为当前群的 `qq_group_send` / `qq_group_send_media` 能产生可见消息或附件。也就是说提示词写错只会导致「该说的没说」，不会变成「不该发的发出去了」。当观察历史与当前消息拼接时，普通消息使用中性的 `Current group message` 标签，明确 @ 消息才使用 `Current addressed message`，不会再把未 @ 的 Direct 消息标成“已明确寻址”。

`group_send_descriptions.send` 和 `group_send_descriptions.send_media` 替换这两个工具在模型工具列表里的描述文本。语义与 `group_prompts` 一致：字段不存在时使用镜像内置描述；字段为字符串时完整替换；设为 `""` 时该工具不带描述。内置描述包含“只在发言确实能改善对话时才调用”这类编辑倾向，如果它与自定义人格的语气冲突，可以在这里改写，而不必自己改镜像。内置描述也已说明「被 @ 时同样只能靠这个工具说话」，自己改写时不要写成“只在没人 @ 你时才用”，否则会和 `direct` 的 @ 回合契约互相打架。
...
```yaml
image: ghcr.io/ichaivalx/hermes-agent-cpa:v2026.8.13-cpa.31
```
