...
- 自定义补丁版本：`25`
- 镜像：`ghcr.io/ichaivalx/hermes-agent-cpa:v2026.8.13-cpa.25`
...
5. `direct` 模式下明确 @ 的消息走同一条可见发送边界：它的普通 final、流式片段和中途文本同样不外发，也必须调用发送工具才能说话。区别只在于 @ 回合保留全部交互面（审批、Clarify、输入中提示、工具进度、排队与卡死通知），普通未 @ 消息则连这些也一并静默。这样一个共享群会话内不再存在两套投递契约，模型也不会把「本以为是私有想法」的一段话在 @ 回合直接播出去。网关自身产生的失败提示（`⚠️ …`）是例外：@ 回合仍会发出，避免每次 API 报错都变成对提问者的沉默；未 @ 回合继续静默失败。
...
```yaml
image: ghcr.io/ichaivalx/hermes-agent-cpa:v2026.8.13-cpa.25
```
